...
function SendPredictionModal({user, gameID, gameInfo, gameData, contract, showSendPredictionModal, setShowSendPredictionModal, reRender, setRerender}) {
                ...
                <h2>Send a Prediction</h2>
                    <div>
                        ...
                        <div class="floatRight">
                            ...
                            <input type="radio" name="team[]" value={gameInfo.liveData.boxscore.teams.away.team.name} onChange={event => setPredictedTeam(event.target.value)} required></input> 
                        </div> 
                    </div> 
                    <div>
                        <button onClick={() => sendPrediction()} disabled={waitingTransaction}>Predict {predictedTeam} </button>
                        
                    </div> 
                    {waitingTransaction? <div><div><b>The transaction is being processed...</b></div><div><ClipLoader color="#fff" loading={true} /*css={override}*/ size={75} /></div></div> : null}
                    <div>
